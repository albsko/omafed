...
APPS_DIR="$RUNNING_SCRIPT_DIR/install/apps/"

# Find all .sh files in the APPS_DIR, sort them, and store them in an array using mapfile
mapfile -t sh_files < <(find "$APPS_DIR" -type f -name "*.sh" | sort)
...
mapfile -t sorted_files_map_keys < <(for key in "${!sh_files_map[@]}"; do echo "$key"; done | sort)
